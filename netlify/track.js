...
// CallMeBot WhatsApp - set up at callmebot.com with your number
const WHATSAPP_NUMBER = '27748315232';
const CALLMEBOT_API_KEY = 'YOUR_CALLMEBOT_KEY'; // We'll set this up after
...
exports.handler = async (event) => {
  ...
  try {
    ...
    // 2. WhatsApp notification for payment clicks only
    ...
    // 3. WhatsApp for visits (optional - can be noisy, currently disabled)
    // if (event_type === 'visit' && CALLMEBOT_API_KEY !== 'YOUR_CALLMEBOT_KEY') {
    //   const msg = encodeURIComponent(`👁 RetailExpert: New visitor from ${referrer || 'Direct'}`);
    //   await fetch(`https://api.callmebot.com/whatsapp.php?phone=${WHATSAPP_NUMBER}&text=${msg}&apikey=${CALLMEBOT_API_KEY}`);
    // }
